...
export class ProjectController {
  ...
  @Roles('ADMIN')
  @Patch(':id')
  @ApiOperation({ summary: 'Обновить данные проекта, только для Администрации' })
  ...
  @ApiOperation({summary:'Добавить пользователя в проект, только для Администрации'})
  ...
  addUser(@Param('projectId') projectId: string, @Param('userId') userId: string) {
    return this.projectService.updateUserToProject(Number(projectId), Number(userId), true);
  }

  @UseGuards(JwtAuthGuard, RolesGuard)
  @Roles('ADMIN')
  @Patch(':projectId/remove-user/:userId')
  @ApiOperation({summary:'Удалить пользователя из проекта, только для Администрации'})
  @ApiParam({ name: 'projectId', example: 1, description: 'ID проекта' })
  @ApiParam({ name: 'userId', example: 5, description: 'ID пользователя' })
  ...
  removeUser(@Param('projectId') projectId: string, @Param('userId') userId: string) {
    return this.projectService.updateUserToProject(Number(projectId), Number(userId), false);
  }
}
